...
const blogContent = {
  ...
  blog: [
    ...
    },
    {
      ...
      title: '今年の最終日について',
      description:
        '2022年の通常授業は28日（木）が最終日になります。年末年始の特別講習および冬期講習は時間割通り行います。自習室は年末年始も含め朝6：00から夜10：30まで利用可能です。どうぞご利用ください。',
      date: '2023年12月',
    },
  ],
};
    ...
    max-md:py-10">
      <div className="container px-4 mx-auto">
          ...
          {blogContent.blog.map((item, index) => (
            <SwiperSlide key={index} className="overflow-visible h-full">
              <div className="p-5 rounded-lg bg-white relative mt-10 m-3">
                <Link href={item.link} className="relative -mt-10 inline-block overflow-hidden rounded-lg mb-4">
                  <Image src={item.img} width={782} height={467} alt="blogImg" className="h-60 object-cover" />
                </Link>
        ...
        <div className="mx-auto flex-col items-center justify-center w-fit">
          <Link href={''} className="duration-300 transition-all ease-in-out py-3 px-6 flex border rounded-full space-x-3 items-center hover:border-gray-400">
            過去のニュースは
